(function ($) {
  ...
  */
  ...
  function aboutAccordion() {
    $('.st-accordion-header').on('click', function () {
      var $item = $(this).closest('.st-accordion-item');
      var $accordion = $item.closest('.st-about-accordion');
      if ($item.hasClass('st-accordion-active')) {
        $item.removeClass('st-accordion-active');
      } else {
        $accordion.find('.st-accordion-item').removeClass('st-accordion-active');
        $item.addClass('st-accordion-active');
      }
    });
  }
  ...
  function mainMenu() {
    ...
    $('.st-munu-toggle').on('click', function () {
      $(this).toggleClass("st-toggle-active");
      $('body').toggleClass("st-mobile-menu-open");
      $(this).siblings('.st-nav-list').slideToggle(300);
    });
  ...
  function onePageNavigation() {
    // Click To Go Top
    $('.st-smooth-move').on('click', function (e) {
      e.preventDefault();
      var thisAttr = $(this).attr('href');
      var $target = $(thisAttr);
      if ($target.length && thisAttr.indexOf('#') === 0) {
        if ($(window).width() < 992) {
          $('body').removeClass('st-mobile-menu-open');
          $('.st-munu-toggle').removeClass('st-toggle-active');
          $(this).closest('.st-nav').find('.st-nav-list').slideUp(300);
        }
        var scrollPoint = $target.offset().top - 10;
        $('body,html').animate({
  ...
  function formValidation() {
    if ($.exists('#contact-form')) {
      ...
      $('#contact-form').on('submit', function (e) {
        ...
        if (!regex.test(email)) {
          $('#st-alert').fadeIn().html('<div class="alert alert-danger"><strong>Uyarı!</strong> Geçerli bir e-posta adresi girin.</div>');
          return false;
        }

        if (name === '' || email === '' || msg === '') {
          $('#st-alert').fadeIn().html('<div class="alert alert-danger"><strong>Uyarı!</strong> Tüm alanları doldurun.</div>');
          return false;
        }
